...
class CameraV2ViewModel(context: Context) : BaseObservable() {
    ...
    fun getCaptureDelay() = DELAY_DURATIONS[delayDurationIndex % DELAY_DURATIONS.size].toString()
    ...
    fun onClickCaptureButton(view: View) {
        if (isTimerOn()) {
            ...
            captureTimer = DELAY_DURATIONS[delayDurationIndex % DELAY_DURATIONS.size]
    ...
    private fun decrementTimer(captureID: Int) {
        ...
        if (captureTimer == 0) {
            ...
            delayMessageLabel.set(captureTimer)
            Log.d(CameraV2ViewModel::class.java.simpleName, "$captureTimer")
